...
```sh
npm install git+ssh://git@github.com:cruisr/odysol-nitro-api.git
```
...
const client = new NitroAPI({
  ...
  password: process.env['NITRO_API_PASSWORD'], // This is the default and can be omitted
});

const cruise = await client.cruise.create();

console.log(cruise.data);
```
...
const cruise: NitroAPI.CruiseCreateResponse = await client.cruise.create();
```
...
<!-- prettier-ignore -->
```ts
const cruise = await client.cruise.create().catch(async (err) => {
  if (err instanceof NitroAPI.APIError) {
...
const response = await client.cruise.create().asResponse();
console.log(response.headers.get('X-My-Header'));
console.log(response.statusText); // access the underlying Response object

const { data: cruise, response: raw } = await client.cruise.create().withResponse();
...
1. Via the `NITRO_API_LOG` environment variable
...
const client = new NitroAPI({
  logger: logger.child({ name: 'NitroAPI' }),
