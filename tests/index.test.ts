...
import { APIPromise } from 'nitro-api/core/api-promise';

import util from 'node:util';
import NitroAPI from 'nitro-api';
import { APIUserAbortError } from 'nitro-api';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['NITRO_API_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: NitroAPI) => {
    ...
    test('default logLevel is warn', async () => {
      const client = new NitroAPI({ username: 'My Username', password: 'My Password' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['NITRO_API_LOG'] = 'debug';
      const client = new NitroAPI({ logger: logger, username: 'My Username', password: 'My Password' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['NITRO_API_LOG'] = 'not a log level';
      const client = new NitroAPI({ logger: logger, username: 'My Username', password: 'My Password' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'NITRO_API_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['NITRO_API_BASE_URL'] = undefined;
    });
    ...
    test('env variable', () => {
      process.env['NITRO_API_BASE_URL'] = 'https://example.com/from_env';
      const client = new NitroAPI({ username: 'My Username', password: 'My Password' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['NITRO_API_BASE_URL'] = ''; // empty
      const client = new NitroAPI({ username: 'My Username', password: 'My Password' });
      expect(client.baseURL).toEqual('https://sandbox.nitroapi.odysol.com/NApi');
    });

    test('blank env variable', () => {
      process.env['NITRO_API_BASE_URL'] = '  '; // blank
      const client = new NitroAPI({ username: 'My Username', password: 'My Password' });
      expect(client.baseURL).toEqual('https://sandbox.nitroapi.odysol.com/NApi');
    });
    ...
    test('in request options overridden by env variable', () => {
      process.env['NITRO_API_BASE_URL'] = 'http://localhost:5000/env';
      const client = new NitroAPI({ username: 'My Username', password: 'My Password' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new NitroAPI({ maxRetries: 4, username: 'My Username', password: 'My Password' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new NitroAPI({ username: 'My Username', password: 'My Password' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['NITRO_API_USERNAME'] = 'My Username';
    process.env['NITRO_API_PASSWORD'] = 'My Password';
    const client = new NitroAPI();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['NITRO_API_USERNAME'] = 'another My Username';
    process.env['NITRO_API_PASSWORD'] = 'another My Password';
    const client = new NitroAPI({ username: 'My Username', password: 'My Password' });
    expect(client.username).toBe('My Username');
    expect(client.password).toBe('My Password');
  });
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new NitroAPI({ username: 'My Username', password: 'My Password', fetch: testFetch });
